...
public abstract class TraversalStrategyInterface {
    ...
    protected void printPath() {
        ...
        for (Vertex vertex : traversalPath) {
            visitedPath.append(vertex).append(' ').append("Distance: ")
                    .append(getDistanceToVertex(getGraph().getVertices().indexOf(vertex))).append(' ').append("\n");
        }
    ...
    protected void printVisitTree() {
        ...
        for (int i = 1; i < getGraph().getNumberOfVertices(); i++) {
            ...
            treeStringBuilder.append(getGraph().getVertices().get(i).getName()).append(" \t")
                    .append(getDistanceToVertex(i)).append('\n');
        }
